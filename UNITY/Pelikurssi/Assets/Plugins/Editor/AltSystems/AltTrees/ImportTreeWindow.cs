...
namespace AltSystems.AltTrees.Editor
{
    ...
    {
        static public GameObject[] trees;
        static AltTreesDataLinks dataLinks = null;
        static int status = 0;
        static Texture2D texture;
        static int typeTree = 0;
        static int indexTree = 0;
        static int convertedAltTrees = 0;

        [MenuItem("Assets/Convert to AltTree")]
        static void menu()
        {
            trees = new GameObject[1] { Selection.gameObjects[0] };
            status = 0;
            texture = null;
            typeTree = 0;
            mats.Clear();
        ...
        [MenuItem("Assets/Convert Multiple Objects to AltTrees (default settings)")]
        static void convertAll()
        {
            trees = Selection.gameObjects;
            status = 0;
            texture = null;
            typeTree = 0;
            mats.Clear();
        ...
        [MenuItem("Assets/Convert Multiple Objects to AltTrees (default settings)", true)]
        static bool convertAllValidate()
        {
            if (Selection.gameObjects != null && Selection.gameObjects.Length > 1)
            {
                for (int i = 0; i < Selection.gameObjects.Length; i++)
                {
                    if (Selection.gameObjects[i] != Selection.gameObjects[i].transform.root.gameObject)
                        return false;
                    if (!(Selection.gameObjects[i].GetComponent<Tree>() != null || Selection.gameObjects[i].GetComponent<LODGroup>() != null || (Selection.gameObjects[i].GetComponent<MeshFilter>() != null && Selection.gameObjects[i].GetComponent<MeshFilter>().sharedMesh != null)))
                        return false;
                }
                return true;
            }
        ...
        void OnGUI()
        {
            ...
            if (trees == null || trees[indexTree] == null)
            {
                this.Close();
                return;
            }
            ...
            if (status == 2)
            {
                ...
                AltTree treeTemp = dataLinks.getAltTree(trees[indexTree]);
            ...
            else if (status == 0)
            {
                ...
                {
                    cco++;
                    texture = (Texture2D)AssetPreview.GetAssetPreview(trees[indexTree]);

                    if (cco == 5)
                    {
                        cco = 0;
                        AssetDatabase.ImportAsset(AssetDatabase.GetAssetPath(trees[indexTree]), ImportAssetOptions.ForceUpdate);
                    }
                    Repaint();
                }
                ...
                useDefaultSettingsConvertTrees = GUI.Toggle(new Rect(261, 288, 200, 25), useDefaultSettingsConvertTrees, "Use the default settings for the");
                GUI.Label(new Rect(276, 302, 200, 25), "following trees");
        ...
        static bool isDeletePrototypesFromTerrain = false;
        static bool isDeleteTreesFromAltTrees = false;
        static bool useDefaultSettingsConvertTrees = false;

        static public void ConvertTree(GameObject _tree, AltTrees_Editor _ate, Terrain _terrainTempImport, bool _isDeleteTreesFromTerrain, bool _isDeleteTreesFromAltTrees, bool _isDeletePrototypesFromTerrain, bool _useDefaultSettingsConvertTrees)
        {
            trees = new GameObject[1] { _tree };
            ate = _ate;
            isImport = true;
            terrainTempImport = _terrainTempImport;
            isDeleteTreesFromTerrain = _isDeleteTreesFromTerrain;
            isDeletePrototypesFromTerrain = _isDeletePrototypesFromTerrain;
            isDeleteTreesFromAltTrees = _isDeleteTreesFromAltTrees;
            useDefaultSettingsConvertTrees = _useDefaultSettingsConvertTrees;
        ...
        static void Convert()
        {
            ...
            if (dataLinks.getAltTree(trees[indexTree]) == null && status == 1 && typeTree != 0)
            {
                if(trees.Length > 1)
                    EditorUtility.DisplayProgressBar("Convert tree (" + (indexTree + 1) + " / " + trees.Length + ")... ", "Converting... ", 0.0f);
                else
                ...
                if (AssetDatabase.GenerateUniqueAssetPath("Assets/Plugins/AltSystems/AltTrees/DataBase/Trees/" + trees[indexTree].name + ".prefab").Equals("Assets/Plugins/AltSystems/AltTrees/DataBase/Trees/" + trees[indexTree].name + ".prefab"))
                    assetPathAndName = "Assets/Plugins/AltSystems/AltTrees/DataBase/Trees/" + trees[indexTree].name + ".prefab";
                else
                    assetPathAndName = AssetDatabase.GenerateUniqueAssetPath("Assets/Plugins/AltSystems/AltTrees/DataBase/Trees/" + trees[indexTree].name + "_2.prefab");

                string[] nameTreeArray = assetPathAndName.Split('/');
                nameTreeArray = nameTreeArray[nameTreeArray.Length - 1].Split('.');
                string folderMat = "";

                if (AssetDatabase.GenerateUniqueAssetPath("Assets/Plugins/AltSystems/AltTrees/DataBase/Trees/TreeResources/" + trees[indexTree].name).Equals("Assets/Plugins/AltSystems/AltTrees/DataBase/Trees/TreeResources/" + trees[indexTree].name))
                    folderMat = "Assets/Plugins/AltSystems/AltTrees/DataBase/Trees/TreeResources/" + trees[indexTree].name;
                else
                    folderMat = AssetDatabase.GenerateUniqueAssetPath("Assets/Plugins/AltSystems/AltTrees/DataBase/Trees/TreeResources/" + trees[indexTree].name + "_2");
                ...
                if (typeTree == 1)
                {
                    ...
                    if (Directory.Exists("AltSystems/AltTrees/SpeedTreeWindParameters") && File.Exists("AltSystems/AltTrees/SpeedTreeWindParameters/" + trees[indexTree].name + ".altWSTParams"))
                    {
                        treeTemp.windParams_ST = new float[704];
                        treeTemp.windParamsUp_ST = new bool[704];

                        using (BinaryReader reader = new BinaryReader(File.Open("AltSystems/AltTrees/SpeedTreeWindParameters/" + trees[indexTree].name + ".altWSTParams", FileMode.Open)))
                        {
                            for (int i = 0; i < 704; i++)
                            {
                ...
                LODGroup lodGroupTemp = trees[indexTree].GetComponent<LODGroup>();
                Material[] matsTemp;
                float sizeTemp = 0f;
                    ...
                    for (int j = 0; j < lodsTemp.Length; j++)
                    {
                        if (lodsTemp[j] != null)
                        {
                            ...
                            if (j == 0)
                            {
                                ...
                                if (isObject && lodTemp.GetComponent<MeshFilter>().sharedMesh.triangles.Length < 500)
                                    treeTemp.gpuInstancing = true;
                                else
                                    treeTemp.gpuInstancing = false;
                            }
                ...
                else
                {
                    ...
                    GameObject lodTemp = (GameObject)Instantiate(trees[indexTree].gameObject, Vector3.zero, Quaternion.identity);
                    lodTemp.transform.parent = goTemp.transform;
                    lodTemp.name = lodTemp.name.Replace("(Clone)", "");
                ...
                foreach (Transform tr in trees[indexTree].transform)
                {
                ...
                if (lodGroupTemp == null)
                {
                    if (treeTemp.lods[0].GetComponent<BoxCollider>() != null || treeTemp.lods[0].GetComponent<CapsuleCollider>() != null || treeTemp.lods[0].GetComponent<SphereCollider>() != null || treeTemp.lods[0].GetComponent<MeshCollider>() != null)
                    {
                        GameObject collGO = new GameObject(treeTemp.lods[0].name);
                        collGO.transform.parent = coll.transform;
                        collGO.transform.localPosition = treeTemp.lods[0].transform.localPosition;
                        collGO.transform.localRotation = treeTemp.lods[0].transform.localRotation;
                        collGO.transform.localScale = treeTemp.lods[0].transform.localScale;

                        if (treeTemp.lods[0].GetComponent<BoxCollider>() != null)
                        {
                            if (UnityEditorInternal.ComponentUtility.CopyComponent(treeTemp.lods[0].GetComponent<BoxCollider>()))
                            {
                                UnityEditorInternal.ComponentUtility.PasteComponentValues(collGO.AddComponent<BoxCollider>());
                            }
                            DestroyImmediate(treeTemp.lods[0].GetComponent<BoxCollider>());

                            EditorUtility.SetDirty(treeTemp.lods[0]);
                            AssetDatabase.SaveAssets();
                            AssetDatabase.Refresh();
                        }
                        if (treeTemp.lods[0].GetComponent<CapsuleCollider>() != null)
                        {
                            if (UnityEditorInternal.ComponentUtility.CopyComponent(treeTemp.lods[0].GetComponent<CapsuleCollider>()))
                            {
                                UnityEditorInternal.ComponentUtility.PasteComponentValues(collGO.AddComponent<CapsuleCollider>());
                            }
                            DestroyImmediate(treeTemp.lods[0].GetComponent<CapsuleCollider>());

                            EditorUtility.SetDirty(treeTemp.lods[0]);
                            AssetDatabase.SaveAssets();
                            AssetDatabase.Refresh();
                        }
                        if (treeTemp.lods[0].GetComponent<SphereCollider>() != null)
                        {
                            if (UnityEditorInternal.ComponentUtility.CopyComponent(treeTemp.lods[0].GetComponent<SphereCollider>()))
                            {
                                UnityEditorInternal.ComponentUtility.PasteComponentValues(collGO.AddComponent<SphereCollider>());
                            }
                            DestroyImmediate(treeTemp.lods[0].GetComponent<SphereCollider>());

                            EditorUtility.SetDirty(treeTemp.lods[0]);
                            AssetDatabase.SaveAssets();
                            AssetDatabase.Refresh();
                        }
                        if (treeTemp.lods[0].GetComponent<MeshCollider>() != null)
                        {
                            if (UnityEditorInternal.ComponentUtility.CopyComponent(treeTemp.lods[0].GetComponent<MeshCollider>()))
                            {
                                UnityEditorInternal.ComponentUtility.PasteComponentValues(collGO.AddComponent<MeshCollider>());
                            }
                            DestroyImmediate(treeTemp.lods[0].GetComponent<MeshCollider>());
                ...
                dataLinks.addTree(trees[indexTree], (AltTree)AssetDatabase.LoadAssetAtPath(assetPathAndName, typeof(AltTree)), treeTemp.id);
                ...
                if (trees.Length > 1)
                {
                    EditorUtility.DisplayProgressBar("Convert tree (" + (indexTree + 1) + " / " + trees.Length + ")... ", "Generating Texture Billboard... ", 0.5f);

                    ((AltTree)AssetDatabase.LoadAssetAtPath(assetPathAndName, typeof(AltTree))).getTextureBillboard(false, "Convert tree ... ");
                }
                else
                {
                ...
                if (trees.Length == 1)
                {
                    ...
                    else
					{
                        ...
                        ate.isDeletePrototypesFromTerrain = isDeletePrototypesFromTerrain;
                        ate.isDeleteTreesFromAltTrees = isDeleteTreesFromAltTrees;
                        ate.useDefaultSettingsConvertTrees = useDefaultSettingsConvertTrees;
                        trees[indexTree] = null;
                        status = 0;
                        texture = null;
                        indexTree = 0;
                        convertedAltTrees = 0;
                    ...
                    EditorUtility.ClearProgressBar();
                    Debug.Log("Converted AltTrees: " + convertedAltTrees);
				}
                else
                {
                    indexTree++;
                    if (indexTree < trees.Length)
                    {
                        status = 0;
                        texture = null;
                        typeTree = 0;
            ...
            {
                if (dataLinks.getAltTree(trees[indexTree]) != null)
                {
                    if (trees.Length == 1)
                        status = 2;
                    else
                ...
                else
                {
                    ...
                    {
                        if (trees[indexTree].GetComponent<Tree>() != null)
                            typeTree = 2;
                        else
                            typeTree = 3;

                        if (trees[indexTree].GetComponent<MeshRenderer>() != null && trees[indexTree].GetComponent<MeshRenderer>().sharedMaterials.Length > 0)
                        {
                            sizeTemp = Mathf.Max(trees[indexTree].GetComponent<MeshRenderer>().bounds.size.y, Mathf.Max(trees[indexTree].GetComponent<MeshRenderer>().bounds.size.x, trees[indexTree].GetComponent<MeshRenderer>().bounds.size.z));
                            
                            matsTemp = trees[indexTree].GetComponent<MeshRenderer>().sharedMaterials;
                            for (int g = 0; g < matsTemp.Length; g++)
                            {
                                if (!mats.Contains(matsTemp[g]))
                                    mats.Add(matsTemp[g]);
                ...
                if (trees.Length == 1 && !useDefaultSettingsConvertTrees)
                {
