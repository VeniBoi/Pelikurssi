...
namespace AltSystems.AltTrees.Tools
{
    public class GenerateSpeedTreeWindParamsFile : MonoBehaviour
    {
                ...
                GUILayout.BeginVertical(GUILayout.Width(600));
                {
                    ...
                    scroll = GUILayout.BeginScrollView(scroll);
                    {
                        ...
                        else
                        {
                            for (int i = 0; i < filesTemp.Length; i++)
                            {
                                string[] strs = filesTemp[i].Replace(".spm", "").Split(Path.DirectorySeparatorChar);
                                if (idZamer != 11)
        ...
        void StartScan()
        {
			...
			{
				GameObject goTemp = AssetDatabase.LoadAssetAtPath(files[j], typeof(GameObject)) as GameObject;
				if(goTemp != null)
					countTemp++;
			}
            TreePrototype[] treePrototypes = new TreePrototype[countTemp];
            int i = 0;
            foreach (string str in files)
            {
				GameObject goTemp = AssetDatabase.LoadAssetAtPath(str, typeof(GameObject)) as GameObject;
				if(goTemp != null)
				{
					treePrototypes[i] = new TreePrototype();
					treePrototypes[i].prefab = goTemp;
					i++;
				}
            }
