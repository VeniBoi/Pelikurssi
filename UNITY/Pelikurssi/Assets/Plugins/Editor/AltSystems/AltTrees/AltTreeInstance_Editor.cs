        ...
        void OnDisable()
        {
            ...
            if (atArray != null && atArray.Length > 0)
            {
                for (int i = 0; i < atArray.Length; i++)
                {
                    ...
                    if(atArray[i].treesNoGroupCount > atArray[i].treesNoGroupEmptyCount)
                        isStopDelete = true;
            ...
            for (int i = 0; i < obj.manager.altTreesMain.altTreesManagerData.patches.Length; i++)
            {
                if (obj.manager.altTreesMain.altTreesManagerData.patches[i].tempTrees != null && obj.manager.altTreesMain.altTreesManagerData.patches[i].tempTrees.Count > 0)
                {
                    ...
                    obj.manager.altTreesMain.altTreesManagerData.patches[i].Init(obj.manager, obj.manager.altTreesMain, obj.manager.altTreesMain.altTreesManagerData, true);
                    ...
                    for (int j = 0; j < obj.manager.altTreesMain.altTreesManagerData.patches[i].tempTrees.Count; j++)
                    {
                        if (!dataLinks.getAltTree(obj.manager.altTreesMain.altTreesManagerData.patches[i].tempTrees[j].idPrototype).isObject)
                        {
                            ...
                            obj.manager.altTreesMain.altTreesManagerData.patches[i].checkTreePrototype(obj.manager.altTreesMain.altTreesManagerData.patches[i].trees[treesBillbSch].idPrototype, dataLinks.getAltTree(obj.manager.altTreesMain.altTreesManagerData.patches[i].trees[treesBillbSch].idPrototype), true, true);
                            treesBillbSch++;
                        }
                        else
                        {
                            addTreesNoBillb.Add(j);
                    ...
                    obj.manager.altTreesMain.altTreesManagerData.patches[i].EditDataFileTrees(null, addTreesBillb.Count, null, -1);
                    //obj.manager.altTreesMain.altTreesManagerData.patches[i].EditDataFileObjects(true, null, addTreesNoBillb.Count, null, -1);

                    obj.manager.addPatch(obj.manager.altTreesMain.altTreesManagerData.patches[i]);

                    AddTreesStruct[] addT = new AddTreesStruct[treesNoBillb];
                    for (int j = 0; j < treesNoBillb; j++)
                    {
                        AltTreesTrees att = obj.manager.altTreesMain.altTreesManagerData.patches[i].tempTrees[addTreesNoBillb[j]];
                        addT[j] = new AddTreesStruct(att.getPosWorld(), att.idPrototype, att.isObject, att.rotation, att.heightScale, att.widthScale, att.color, att.colorBark);
                    }

                    obj.manager.altTreesMain.addTrees(addT, true);

                    obj.manager.altTreesMain.altTreesManagerData.patches[i].tempTrees.Clear();
                }
            }
