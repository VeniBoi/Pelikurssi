...
public class FloatingOrigin : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if (cameraPosition.magnitude > threshold)
        {
            ...
            foreach(Object o in objects)
            {
                ...
                if (t.parent == null)
                {
                    if (t.name != "AltTrees")
                        ...
                        t.GetComponent<AltSystems.AltTrees.AltTrees>().setFloatingOriginJump(cameraPosition);
                }
            }
            ...
            foreach (UnityEngine.Object o in objects)
            {
                ...
                #if !(UNITY_5_2 || UNITY_5_3 || UNITY_5_4)
		            particlesNeeded = sys.main.maxParticles;
                #else
                    particlesNeeded = sys.maxParticles;
                #endif
    ...
    void OnGUI()
    {
        if (drawGUI)
        {
            GUI.BeginGroup(new Rect(10, Screen.height - 55, 400, 50));
            {
                GUI.Box(new Rect(0, 0, 400, 50), "");
                GUI.Label(new Rect(5, 5, 400, 21), "Floating Origin enabled.");
                GUI.Label(new Rect(5, 24, 400, 21), "Camera coordinates: " + gameObject.transform.position.ToString());
            }
            GUI.EndGroup();
        }
    }
